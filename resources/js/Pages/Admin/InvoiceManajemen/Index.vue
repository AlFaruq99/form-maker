                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-1 gap-6">
                    <div class="card bg-white p-4 shadow grid grid-cols-1 gap-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label v-if="!url" for="dropzone-file" class="cursor-pointer flex flex-col items-center rounded-xl border-2 border-dashed border-blue-400 bg-white py-5 text-center min-h-40">
                                ...
                                <div id="preview" class="flex gap-4">
                                    <img class="max-h-40" v-if="url" :src="url" />
                                    <button class="rounded-lg btn btn-outline btn-error" v-if="url" @click="clearUrl">x</button>
                                </div>
                            </div>
    ...
    methods: {
        ...
        async createHandler() {
            try {
                ...
                const response = await axios.post(route('panel.invoice.create'), invoiceData);
