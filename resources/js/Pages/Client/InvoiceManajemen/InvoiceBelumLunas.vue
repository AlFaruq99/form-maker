<template>
   <div class=" grid grid-cols-1 w-full gap-6 col-span-3">
        <div class="card bg-white p-6 grid grid-cols-1 gap-6">
            <p class="font-semibold">Belum bayar</p>
            <div class="container">
                <Link :href="route('client.invoice.clientCreate',{_query:{
                    status: 'belum_bayar'
                            ...
                            <tr class=" whitespace-nowrap" v-for="(item, index) in invoiceData.data" :key="index">
                                ...
                                <td>
                                    <div class="inline-flex space-x-2">
                                        ...
                                        <ButtonSendMail :user-level="level" :data-id="item.id"></ButtonSendMail>
                                        <div class="divider divider-horizontal"></div>
                                        <a :href="route('client.invoice.preview',item.id)" target="_blank">
                                            <svg xmlns="http://www.w3.org/2000/svg"  class="w-6 text-orange-400" viewBox="0 0 24 24"><path fill="currentColor" d="M12 9a3 3 0 0 0-3 3a3 3 0 0 0 3 3a3 3 0 0 0 3-3a3 3 0 0 0-3-3m0 8a5 5 0 0 1-5-5a5 5 0 0 1 5-5a5 5 0 0 1 5 5a5 5 0 0 1-5 5m0-12.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5"/></svg>
                                        </a>
                                        <a :href="route('client.invoice.download',item.id)" class="tooltip" data-tip="Unduh">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 text-orange-600" viewBox="0 0 24 24"><path fill="currentColor" d="M5 20h14v-2H5zM19 9h-4V3H9v6H5l7 7z"/></svg>
                                        </a>
                                    </div>
                                </td>
                                <td >
                                ...
                                <td>{{ item?.total }}</td>
                                <td>
                                    <Link :href="route('client.user.userActivation',{
                                        id: item.id
...
import ButtonSendMail from '@/Components/ButtonSendMail.vue'

export default {
    components:{
        TableVue, Head, Link,Toast,ButtonSendMail
    },
    props:{
        userId:Number,
        level:String
    ...
    methods: {
        ...
        async fetchInvoice(urlParam){
            ...
            if (urlParam) {
                ...
                url  = route('client.invoice.fetchInvoice',{_query:{
                    status : 'belum_bayar',
                    length : this.length,
                    search : this.search,
                    user_id: this.userId
        ...
        async sendWhastappMedia(item){
            try {
                ...
                await axios.get(
                    route('client.invoice.sendMedia',{
                        id: item.id,
