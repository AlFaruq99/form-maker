<template>
    <div class="space-y-6">
        <label for="bacgroundInput" class="btn w-fit btn-outline btn-primary text-white">
        ...
        <div class="space-y-2">
            <div class="container form-group grid grid-cols-1 gap-2">
                ...
                <textarea v-model="description" class="textarea text-black textarea-bordered min-h-5 max-h-24 max-w-xs" placeholder="Ketik deskripsi Anda di sini"></textarea>
            </div>
            <div class="container form-group grid grid-cols-1 gap-2">
                <label for="judul" class="text-sm font-semibold">Judul</label>
        ...
        <div>
            ...
            <p class=" text-xl font-bold">Isi formulir</p>
            <div></div>
            <div v-if="content.length < 1">
                <p class=" font-thin">Tambah kolom untuk data dengan tombol tambah kolom.</p>
            </div>
            <div  v-for="(item, index) in content" :key="index">
                <div class="container formulir-body grid grid-cols-1 border-b py-3">
        ...
        <div class="container inline-flex py-2 justify-between">
            <button class="btn bg-emerald-500 text-white" @click="pushContentHandler"> 
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5" viewBox="0 0 21 21">
                    <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M17 4a2.121 2.121 0 0 1 0 3l-9.5 9.5l-4 1l1-3.944l9.504-9.552a2.116 2.116 0 0 1 2.864-.125zM9.5 17.5h8m-2-11l1 1"/>
                </svg>
                Tambah kolom
            </button>
        </div>
